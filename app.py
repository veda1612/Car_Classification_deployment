...
def predict():    
    if request.method == 'POST':
        ...
        prediction=model.predict([[Price,MaintenanceCost,NumberOfDoors,Capacity,SizeOfLuggageBoot]])
        output=prediction
        if output<0:
            return render_template('index.html',prediction_texts="Sorry prediction has some problem")
        elif output==0:
            return render_template('index.html',prediction_text="unacc")
        elif output==1:
            return render_template('index.html',prediction_text="acc(")
        elif output==2:
            return render_template('index.html',prediction_text="good")
        elif output==3:
            return render_template('index.html',prediction_text="vgood")
    else:
        return render_template('index.html')
